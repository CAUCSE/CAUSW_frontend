...
export class SettingRoleAlumniPageUiStore implements WithSearchUserModalUi {
  submitDisabled = true;
  target?: Model.User;
  ...
  *update(target: Model.User, source: Model.User): Generator {
    try {
      this.submitDisabled = true;
      yield Repo.updateRole(target.id, source.role);
...
export const PageUiStoreImpl = new SettingRoleAlumniPageUiStore();
